/* ============================================================
   ...
   Conexão direta com Supabase (sem backend)
   ============================================================ */
...
async function sbPost(table, body) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}`, {
    method: 'POST',
    headers: { 'apikey': SUPABASE_ANON, 'Authorization': `Bearer ${_sbToken || SUPABASE_ANON}`, 'Content-Type': 'application/json', 'Prefer': 'return=representation' },
...
async function sbUpload(file, bucket) {
  ...
  const res  = await fetch(`${SUPABASE_URL}/storage/v1/object/qualiconsig/${path}`, {
    method: 'POST',
    headers: { 'apikey': SUPABASE_ANON, 'Authorization': `Bearer ${_sbToken || SUPABASE_ANON}`, 'Content-Type': file.type },
    body: file
  });
  if (!res.ok) return null;
...
function doLogout() {
  _sbToken = null;
  localStorage.removeItem('qcms_sb_token');
...
async function loadAll() {
  await Promise.all([loadDashboard(), loadBanners(), loadCards(), loadWhyItems(), loadServices(), loadPosts(), loadContacts(), loadUsers(), loadSettings(), loadTexts()]);
}

/* ===== DASHBOARD ===== */
async function loadDashboard() {
  const [b, p, c, u] = await Promise.all([
    sbGet('banners', '?is_active=eq.true&select=id'),
    sbGet('posts', '?status=eq.published&select=id'),
    sbGet('contacts', '?select=id'),
    sbGet('cms_users', '?is_active=eq.true&select=id'),
  ]);
...
async function loadBanners() {
  const data = await sbGet('banners', '?order=order_position.asc&select=*');
...
async function saveBanner() {
  ...
  const body = { title: document.getElementById('b-title').value, subtitle: document.getElementById('b-subtitle').value, cta_text: document.getElementById('b-cta-text').value, cta_url: document.getElementById('b-cta-url').value, image_url: imgUrl, image_position: document.getElementById('b-img-position').value || 'center center', is_active: document.getElementById('b-active').classList.contains('on') };
  const res = id ? await sbPatch('banners', id, body) : await sbPost('banners', {...body, order_position: _banners.length});
  if (res) { closeModal('modal-banner'); loadBanners(); toast('Banner salvo!', 'success'); }
  else toast('Erro ao salvar banner', 'error');
}
async function deleteBanner(id) {
  if (!confirm('Excluir este banner?')) return;
  await sbDelete('banners', id); loadBanners(); toast('Banner excluído');
}
async function saveBannerOrder() {
  const items = document.querySelectorAll('#banners-order-list .order-item');
  const updates = Array.from(items).map((el, i) => sbPatch('banners', el.dataset.id, { order_position: i }));
...
function updateBannerPreview() {
  ...
  if (bg) { bg.style.backgroundImage = url ? `url('${url}')` : 'none'; bg.style.backgroundPosition = pos; }
  const pt = document.getElementById('prev-title');
  const ps = document.getElementById('prev-subtitle');
  if (pt) pt.textContent = document.getElementById('b-title').value || 'Título do Banner';
  if (ps) ps.textContent = document.getElementById('b-subtitle').value || 'Subtítulo...';
}
...
function renderServices() { document.getElementById('services-body').innerHTML=_services.map(s=>`<tr><td><img class="table-img" src="${s.image_url||''}" onerror="this.style.opacity='0.1'"></td><td><strong>${esc(s.name)}</strong></td><td>${esc(s.link_url||'—')}</td><td><span class="badge badge-success">Ativo</span></td><td style="display:flex;gap:6px"><button class="btn-icon" onclick="editService('${s.id}')"><i class="fas fa-edit"></i></button><button class="btn-icon danger" onclick="deleteService('${s.id}')"><i class="fas fa-trash"></i></button></td></tr>`).join(''); }
function openServiceModal(id=null) { const s=id?_services.find(x=>x.id===id):null; document.getElementById('service-modal-title').textContent=s?'Editar Serviço':'Novo Serviço'; document.getElementById('s-id').value=s?.id||''; document.getElementById('s-name').value=s?.name||''; document.getElementById('s-link').value=s?.link_url||''; document.getElementById('s-img-url').value=s?.image_url||''; const prev=document.getElementById('s-img-preview'); prev.style.display=s?.image_url?'block':'none'; if(s?.image_url)prev.src=s.image_url; openModal('modal-service'); }
function editService(id) { openServiceModal(id); }
async function saveService() { const id=document.getElementById('s-id').value; const imgFile=document.querySelector('#modal-service input[type="file"]').files[0]; let imgUrl=document.getElementById('s-img-url').value; if(imgFile){const up=await sbUpload(imgFile,'services');if(up)imgUrl=up;} const body={name:document.getElementById('s-name').value,link_url:document.getElementById('s-link').value,image_url:imgUrl}; const res=id?await sbPatch('services',id,body):await sbPost('services',body); if(res){closeModal('modal-service');loadServices();toast('Serviço salvo!','success');} }
...
function previewImage(input, previewId) { const file=input.files[0]; if(!file)return; const reader=new FileReader(); reader.onload=e=>{const img=document.getElementById(previewId);img.src=e.target.result;img.style.display='block';}; reader.readAsDataURL(file); }
